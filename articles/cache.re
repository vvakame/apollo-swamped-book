...
ハマりの説明に入りましょう。
筆者が作っていた（いる）GraphQL APIは、スキーマのない任意のデータ構造をフィールドとして持っています。
そのため、@<code>{scalar JSON}というcustom scalar typeを作成しました（@<list>{code/cache-example/schema.graphql}）。
#@# OK gfx: ↑ この2行ちょとわかりにくかった（ってか以前ちらっと聞いていなければわからなかった）。「筆者が作っていた（いる）GraphQL APIは、スキーマのない任意のデータ構造をフィールドとして持っています。」だとどうでしょう。
...
キャッシュのデータを、キーを変えコピー・保存したくなった場合、Apolloではそれはよくない発想です。
#@# REVIEW OK sota1235: "キャッシュのデータを"の直後に句読点を入れたほうが読みやすそうです
...
代わりに、Queryを拡張して@<code>{current}プリフィクスをつけたscalar型のフィールドを定義して使っています。
#@# OK gfx: ここがよくわからなかった。 `current prefix つきのフィールドは「独自のtype」ではないの？？
...
変数の値の変換をどこかのレイヤーでできるとよいのでしょうが、筆者の知るかぎり難しいようです。
