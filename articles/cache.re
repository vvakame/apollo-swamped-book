...
#@# gfx: ↑ この2行ちょとわかりにくかった（ってか以前ちらっと聞いていなければわからなかった）。「筆者が作っていた（いる）GraphQL APIは、スキーマのない任意のデータ構造をフィールドとして持っています。」だとどうでしょう。
...
#@# gfx: ここがよくわからなかった。 `current prefix つきのフィールドは「独自のtype」ではないの？？
