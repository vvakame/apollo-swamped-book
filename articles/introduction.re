...
このように、GraphQL+Apollo+TypeScriptではアプリケーションで使用する型・フィールドのみを厳密に扱えるようになっています。
ワンダフル！
Open API（Swagger）も似たようなことができますが、アプリケーション側が必要な型とサーバ側が提供する型が厳密に一致することがほぼない点で不便です。
#@# OK gfx: s/GraphQL+apollo/GraphQL+Apollo+TypeScript/
#@# REVIEW OK sota1235: "アプリケーション的に関心がある" => 少しピンと来ないかも(意味は分かる)、"アプリケーションで使用する型のみを厳密に~"とかのほうが言いたいことと近いのかなぁと思いました
#@# REVIEW OK sota1235: 上の文と下の文をつなげたほうがわかりやすいです。"...も似たようなことができますが、アプリケーション側の関心と..."
